...
from modules.loss_functions import HO_energy, HO_energy_2der
import modules.loss_functions
...
# Mesh parameters
npoints = [50, 100, 150] # points per dimension
a = -5
b = 5
...
dims = [3]
hidden_neurons = [10, 20, 50, 100, 200, 300]
actfuns = ['Sigmoid', 'Softplus']
optimizers = ['RMSprop']
learning_rates = [0.005, 0.05, 0.1] # Use decimal notation 
epsilon = 1e-8
smoothing_constants = [0.9]
momenta = [0.0, 0.5, 0.9]
ll = [dims, hidden_neurons, actfuns, optimizers, learning_rates, smoothing_constants, momenta]
...
def parallelize(lists, nchunks_general, which_chunk):
    
    # We decide upon the longest list
    lengths = {len(l):l for l in lists}
    thelongest = lengths.get(max(lengths.keys()))
    
    # We compute how to split that list into chunks
    if nchunks_general > len(hidden_neurons):
        nchunks = len(hidden_neurons)
    else:
        nchunks = nchunks_general
    if which_chunk >= nchunks:
        sys.exit()
    
    return thelongest, nchunks

thelongest, nchunks = parallelize(ll, nchunks_general, which_chunk)
for nhid, N, d, arch, optim, actfun, lr, alpha, mom in product(split(l=thelongest, n=nchunks)[which_chunk],
                                                               npoints, dims, net_archs,
                                                               optimizers, actfuns,
                                                               learning_rates,
                                                               smoothing_constants,
                                                               momenta):            
    print(f'\nN = {N}, D = {d}, Arch = {arch}, Neurons = {nhid}, Actfun = {actfun}, ' \
    ...
    if pretrain:
        ...
        for t in tqdm(range(pre_epochs)):
            ov, loss, psi = modules.loss_functions.overlap(model=psi_ann, # ov is really 1-<ANN|targ>^2
                            target_dD=target_dD,
                            w_i=w_trpz,
                            train_data=mesh)
            ...
            if (t % pre_leap) == 0 and pre_periodic_plots:
    ...
    for t in tqdm(range(epochs)):
        ...
        if t == 0:
            ...
            print(f'\nRight after pretraining: \nOverlap: {overlap}, \nK: {K}, ' \
                  f'\nU: {U}, \nE: {E}\n')
            print(f'Gradients: {[i.grad for i in psi_ann.lc1.parameters()]}')
            """
